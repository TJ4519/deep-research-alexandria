...
| `grep_claude_in_a_box_snapshot_2025_12_11` | `docs/references/claude_in_a_box_grep_agents_sdk_2025_12_11.md` | `EXTERNAL_SNAPSHOT` / `DONOR_ARTIFACT` | Normalized user-provided Markdown snapshot stored; public URL checked 2026-04-21 | A task concerns Grep implementation substrate, Claude Agent SDK, isolated agent workspaces, selective skill loading, MCP tool surfaces, fork/resume session handling, Celery/Temporal execution, or agent.py architecture | This article is implementation evidence for Grep's substrate choices, but it does not automatically ratify Alexandria's runtime substrate; any conflict with the candidate charter requires explicit amendment |
| `claude_in_a_box_agent_py_implications_2026_04_21` | `docs/design-docs/claude_in_a_box_implications_for_alexandria_agent_py_2026_04_21.md` | `LOCAL_TRANSLATION` | Active substrate amendment memo | A task concerns `agent.py`, SDK versus LangGraph choice, branch execution, AgentHarnessConfig, skill loading, MCP allowlists, session fork/resume, transcript custody, or Grep functional-copy strategy | Memo translates donor evidence into Alexandria design pressure; it does not ratify the candidate charter or bypass the Design Document quality bar |
