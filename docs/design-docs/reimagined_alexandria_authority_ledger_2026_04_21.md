...
| Codex-DR sandbox architect handoff | `docs/design-docs/codex_dr_sandbox_architect_handoff_2026_04_22.md` and `sandbox/codex-dr/` | `LOCAL_TRANSLATION` | Active | First architect workspace, bootstrap doctrine, memory contract, benchmark target calibration, terminal-agent sandbox boundary, and service-runtime bridge |
...
sandbox architect work enters through `sandbox/codex-dr/`, completes
`sandbox/codex-dr/docs/BOOTSTRAP_DOCTRINE.md` before provider-backed execution,
and treats Program 90 receipts as historical evidence, not as the new target
definition.
